# [2976. Minimum Cost to Convert String I][link] (Medium)
...
`changed`, and an integer array `cost`, where `cost[i]` represents the cost of changing the
character `original[i]` to the character `changed[i]`.
...
```
Input: source = "abcd", target = "acbe", original = ["a","b","c","c","e","d"], changed =
...
followed by changing the character 'c' to 'b' at a cost of 2, for a total cost of 1 + 2 = 3. To
