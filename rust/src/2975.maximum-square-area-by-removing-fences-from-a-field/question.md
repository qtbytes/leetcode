# [2975. Maximum Square Area by Removing Fences From a Field][link] (Medium)
...
and `(m, 1)` to `(m, n)` and two vertical fences from the coordinates `(1, 1)` to `(m, 1)` and `(1,
n)` to `(m, n)`. These fences **cannot** be removed.
...
Output: 4
Explanation: Removing the horizontal fence at 2 and the vertical fence at 2 will give a square field
of area 4.
```
